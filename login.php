...
if (isset($_POST["login"])) {  // condiç de verificaç de login
    ...
    $sql = "SELECT * FROM usuarios WHERE email = '$email'"; //verif se o email armaz consta no banco
    ...
    if ($result->num_rows == 1) {                  // aqui a verificaç do email, se encontrado $result = 1(vdd) foi encontrado no banco
        ...
        if (password_verify($senha, $usuario["senha"])) {  // para verificar a senha, esta é a função se a senha corresponde a hash senha armaz no banco
            $_SESSION["usuario_id"] = $usuario["id"]; // se a senah estiver correta, esta linha armaz o valor da coluna id do usuário, o ID dele está presentes em todas as pg do site
